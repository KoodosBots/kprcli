...
REM Scan system specifications
echo [INFO] Scanning system specifications...
"%INSTALL_DIR%\ai-form-filler.exe" system-scan > "%INSTALL_DIR%\system-specs.txt"
if %errorLevel% neq 0 (
    echo [WARNING] Could not scan system specifications
    echo [INFO] Default performance settings will be used
) else (
    echo [OK] System specifications scanned successfully
    echo [INFO] Results saved to %INSTALL_DIR%\system-specs.txt
    type "%INSTALL_DIR%\system-specs.txt"
)
...
echo 2. Check system-specs.txt for performance recommendations
echo.
echo 3. Launch "AI Form Filler" from desktop shortcut
